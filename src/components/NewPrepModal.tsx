...
import {
  Modal, View, Text, ScrollView, Pressable, Alert,
...
import { PrepSchema } from '@/api/schemas';
...
export function NewPrepModal({ visible, onClose }: Props) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const newPrep = await createPrep({
        division: division!,
      ...
      // Put the new prep directly in cache so all tabs update instantly
      qc.setQueryData(['prep-current'], PrepSchema.parse(newPrep));
      reset();
      onClose();
    } catch {
      setSubmitting(false);
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
          ...
          >
            ...
            <View style={{ borderRadius: radius.lg, backgroundColor: colors.neutral[50], padding: 16, marginBottom: 16 }}>
              ...
              <Stepper label="Maintenance weeks" value={maintenanceWeeks} min={0} max={8} onChange={setMaintenanceWeeks} suffix="w" />
              <Stepper label="Cut weeks" value={cutWeeks} min={4} max={20} onChange={setCutWeeks} suffix="w" />
            </View>
